...
# ------------------------------------------------------------
# (3.4) change initial gene set
# ------------------------------------------------------------

aging_map = readLines("../af_merge_mice.txt")
aging_gene_glmnet_all_tissue_add_50 <- readRDS("/lustre/user/liclab/liocean/maosl/tms/tms_h5ad/aging_gene_glmnet_all_tissue_add_50.rds")
aging_gene_glmnet_all_tissue <- readRDS("/lustre/user/liclab/liocean/maosl/tms/tms_h5ad/aging_gene_glmnet_all_tissue_500.rds")
aging_gene_change_up <- data.frame(row.names = names(aging_gene_glmnet_all_tissue_add_50))
aging_gene_change_down <- data.frame(row.names = names(aging_gene_glmnet_all_tissue_add_50))

aging_map_sample <- data.frame(row.names = 1:500)
for (i in 1:100){
  aging_map_sample[[i]] <- sample(aging_map, 500) 
}

for (count in 1:100){
  done <- 1
  af_merge <- aging_map_sample[[count]]
  aging_gene_glmnet_all_tissue_add <- list()
  df_glm_cor <- data.frame(row.names = c("train", "test", "mse_train", "mse_test", "lambda.1se"))
  for (h5ad_file in dir("/lustre/user/liclab/liocean/maosl/tms/tms_h5ad/", pattern = "h5ad$"))
  {
    tissue_name <- unlist(strsplit(unlist(strsplit(str_extract(h5ad_file, pattern = "ns-.*\\.h5ad$"), "-"))[2], "\\."))[1]
    if (stringr::str_detect(h5ad_file, "droplet")){
      method <- "droplet" 
    ...
    colnames(aging_genes) <- c("down", "up")
    aging_gene_glmnet_all_tissue_add[[tissue_method]] <- aging_genes
    print(paste("Count:", count, " Have done:", done, "/", total_num, sep = ""))
    flush.console()
    done <- done + 1
  }
  
  for (tissue in names(aging_gene_glmnet_all_tissue_add_50)){
    aging_gene_change_up[tissue, count] <- sum(!(aging_gene_glmnet_all_tissue_add[[tissue]][1:50, "up"] %in% 
                                                   aging_gene_glmnet_all_tissue_add_50[[tissue]][1:50, "up"]))
    aging_gene_change_down[tissue, count] <- sum(!(aging_gene_glmnet_all_tissue_add[[tissue]][1:50, "down"] %in% 
                                                     aging_gene_glmnet_all_tissue_add_50[[tissue]][1:50, "down"]))
  }  
}

aging_gene_change_up_summary <- data.frame(cbind(Matrix::rowMeans(aging_gene_change_up),
                                                 apply(aging_gene_change_up, 1, sd)/sqrt(count)))
colnames(aging_gene_change_up_summary) <- c("mean", "se")
aging_gene_change_up_summary$direction <- "Up-regulated"
aging_gene_change_up_summary$tissue <- row.names(aging_gene_change_up_summary)
aging_gene_change_up_summary[["error_bar_pos"]] <-aging_gene_change_up_summary$mean

aging_gene_change_down_summary <- data.frame(cbind(Matrix::rowMeans(aging_gene_change_down),
                                                   apply(aging_gene_change_down, 1, sd)/sqrt(count)))
colnames(aging_gene_change_down_summary) <- c("mean", "se")
aging_gene_change_down_summary$direction <- "Down-regulated"
aging_gene_change_down_summary$tissue <- row.names(aging_gene_change_down_summary)
aging_gene_change_down_summary$mean <- aging_gene_change_up_summary$mean + aging_gene_change_down_summary$mean
aging_gene_change_down_summary[["error_bar_pos"]] <- aging_gene_change_down_summary$mean + aging_gene_change_up_summary$mean


aging_gene_change <- rbind(aging_gene_change_up_summary, aging_gene_change_down_summary)

ggplot(data = aging_gene_change, mapping = aes(x = tissue, y = mean, fill = direction)) +
  geom_col() +
  geom_errorbar(aes(ymin=error_bar_pos-se, ymax=error_bar_pos+se), width=0.5) +
  geom_point(aes(y=error_bar_pos)) +
  guides(fill=guide_legend(title="Direction")) +
  theme_classic() + 
  scale_fill_npg() +
  labs(y="Number of genes",x="Samples", 
       title = "Aging genes change after initial gene set changing")+
  theme(legend.position = "bottom", 
        axis.text.x = element_text(angle=45, hjust=1, vjust=1))
ggsave("../figure/Aging_genes_change_after_initial_gene_set_changing.pdf", width = 10, height = 5)
